...
class HotelsRepository(BaseRepository):
    ...
    async def get_filtered_by_date(
        ...
        free_hotel_ids = (
            select(RoomsORM.hotel_id).distinct().select_from(RoomsORM).filter(RoomsORM.id.in_(free_room_ids))
        )

        query = select(HotelsORM).select_from(HotelsORM).filter(HotelsORM.id.in_(free_hotel_ids))
        if title:
